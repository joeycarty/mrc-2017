...
<section class="indent">
	...
	<?php while(have_posts()){ the_post(); ?>
		...
		<?php // DISPLAY ABOUT SECTION ?>
		<?php if($about_header && $about_watermark && $about_copy){ ?>
			<section class="about-the-team">
	      <?php if($about_watermark){ ?>
	      	<h2 class="watermark-header undent"><?php echo $about_watermark; ?></h2>
	      <?php } //endif ?>
	      ...
	      <?php } //endif ?>
				...
				<?php if($about_image_object){ ?>
	      	<div class="team-lineup">
		        <img src="<?php echo $about_photo_url; ?>" alt="<?php echo $about_photo_alt; ?>">
		      </div>
	      <?php } //endif ?>
				
				<?php if($about_callout){ ?>
					<aside>
		        <p class="small"><?php echo $about_callout; ?></p>
		      </aside>
	      <?php } //endif ?>
	    </section>
		<?php } //endif ?>
		...
		<?php $testimonial = get_field('testimonial');
			if($testimonial){ 
				$post = $testimonial;	// override $post
				setup_postdata( $post ); ?>
					
					<?php include( locate_template( 'template-parts/part-testimonial.php', false, false ) ); ?>
	    
	    <?php wp_reset_postdata(); // !!! reset the $post object ?>
		<?php }							 	//endif for testimonial; ?>
